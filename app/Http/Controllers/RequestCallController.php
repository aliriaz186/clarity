...
class RequestCallController extends Controller
{
    ...
    {
        $userId = ExpertiseAreaTable::where('id', $journalistId)->first()['id_user'];
        if (PaymentTable::where('user_id', $userId)->exists() && !empty(Session::get('userId'))) {
            $paymentTableData = PaymentTable::where('user_id', $userId)->first();
            ...
            $profileTable = ProfileTable::where('user_id', $userId)->first();
            ...
            $paymentTableData['hourlyRate'] = $profileTable->hourly_rate;
            $hourlyRate = $profileTable->hourly_rate;
            $paymentTableData['hourly15MinutesCost'] = $hourlyRate * 15;
            $paymentTableData['hourly30MinutesCost'] = $hourlyRate * 30;
            $paymentTableData['hourly1HourCost'] = $hourlyRate * 60;
            $paymentTableData['journalistId'] = $journalistId;
    ...
    public function requestACall(Request $request)
    {
        try {
            ...
            if ($charge['status'] == 'succeeded') {
                ...
                $callRequestTable->suggested_time_one = $request->suggestedTime1;
                $callRequestTable->suggested_time_two = $request->suggestedTime2;
                $callRequestTable->suggested_time_three = $request->suggestedTime3;
